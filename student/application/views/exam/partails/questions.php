		...
		foreach($query as $main_id=>$question_arrs){
			foreach($question_arrs  as $index=>$question){
						...
						>
							<div class="form-check form-check-inline">
								<input
									...
									type="checkbox"
									id="bank<?php  echo $question['id'];?>"

								/>
							</div>
							<h6> <?php  echo $question['title'];?></h6>
						</div>
						<div class="question_div<?php  echo $question['id'];?>">
						<!--	<input
				...
				<?php } elseif(isset($question['parent_id']) && $question['parent_id']==0&& count($question_arrs)==1){
					...
					<div
						class="question_div<?php  echo $question['id'];?> d-flex justify-content-between flex-row align-items-start flex-wrap mt-4 pb-3 border-bottom border-solid border-fifth-color"
					>
						<div
							class="d-flex justify-content-start align-items-start flex-row gap-0"
						>
...
>
	...
	>
		...
		<form action="" class="w-100 question_exist_free" id="question_exist_free">
					...
					>
						...
						<div class="w-100">
							...
							>
								...
								<div class="input-group">
									<input name="media[0][]" type="file" class="form-control"  onchange="uploadMedia(this, 0,0)">
						...
						<div class="w-100">
							...
							>
								...
								/>
								<div class="input-group">
									<input name="media[0][]" type="file" class="form-control" onchange="uploadMedia(this, 0,1)">
									<input type="hidden" name="media_paths[0][]" />
									<label class="input-group-text" for="inputGroupFile02">او ارفع ميديا كـ اجابة</label>
								</div>
							</label>
						...
						<div class="w-100">
							...
							>
								...
								/>
								<div class="input-group">
									<input name="media[0][]" type="file" class="form-control"  onchange="uploadMedia(this, 0,2)">
									<input type="hidden" name="media_paths[0][]" />
									<label class="input-group-text" for="inputGroupFile02">او ارفع ميديا كـ اجابة</label>
								</div>
						...
						<div class="w-100">
							...
							>
								...
								/>
								<div class="input-group">
									<input name="media[0][]" type="file" class="form-control"   onchange="uploadMedia(this, 0,3)">
									<input type="hidden" name="media_paths[0][]" />
									<label class="input-group-text" for="inputGroupFile02">او ارفع ميديا كـ اجابة</label>
								</div>
							</label>
...
<script>
    ...
    document.getElementById('add_another_question_free').addEventListener('click', function() {
        ...
        inputField.value='';
        var inputFieldfree_arrange = newQuestion.querySelector('input[name="free_arrange[0][]"]');
        inputFieldfree_arrange.name = `free_arrange[${container.children.length}][]`;
        inputFieldfree_arrange.value='';

        var inputFieldfree_degree= newQuestion.querySelector('input[name="free_degree[0][]"]');
        inputFieldfree_degree.name = `free_degree[${container.children.length}][]`;
        inputFieldfree_degree.value='';
...
<script>
   ...
   // Add a change event listener to a checkbox
        $('input[type="checkbox"]').on('change', function() {
            var checked_id = $(this).attr('id');
            if (!$(this).is(':checked')) {
                var question_selector = checked_id.replace(/\D/g, '');
                $(".question_div" + question_selector).empty();
            }else {
                // Update the checked state of the checkbox
                //    $(this).prop('checked', !$(this).prop('checked'));

                var question_selector = checked_id.replace(/\D/g, '');
                if ($(".question_div" + question_selector).length > 0) {
                    $(".question_div" + question_selector).empty();
                    $(".question_div" + question_selector).append('<input name="arrange[' + question_selector + ']" type="number" class="form-control w-100 mb-3" placeholder="ترتيب السؤال" aria-label="text" required/>' + '<input name="checkdegree[' + question_selector + ']" type="number" class="form-control w-100" placeholder="درجة السؤال" aria-label="text" required/>');
                } else {
                    console.log("Element not found: .question_div" + question_selector);
                }
            }
        });
   // });
