...
class Master extends CI_Controller {
	...
	public function __construct()
    {
		...
		$student_id = $this->session->userdata("student_user_id");
	    $courses = $this->db->query("SELECT * FROM courses WHERE id in (SELECT course_id FROM students_courses WHERE student_id = '$student_id')");
	    $this->data['courses'] = $courses->result();
	    $this->data['title'] = 'دوراتي';

		$user_id = $this->session->userdata('student_user_id');
	    $student = $this->db->query("SELECT * FROM students WHERE id = '$user_id'")->result();
	    $this->data["student"] = $student[0];


		$course = $this->db->query("SELECT * FROM courses WHERE id IN (SELECT course_id FROM students_courses WHERE student_id = '$student_id')")->result();
		...
		$this->data['course_type'] = $course_type[0];


		$this->data["books"] = $this->db->query("SELECT * FROM books WHERE course_type_id = '$course_type_id'")->result();
		...
		$query = $this->db->query("SELECT * FROM students WHERE id = '$student_id'")->result();
		$school_id=$query[0]->school_id;
		$teachers = $this->db->query("SELECT * FROM teachers WHERE id IN (SELECT teacher_id FROM courses_teachers WHERE course_id = '$course_id')")->result();
		//var_dump($teachers);
		$this->data["teachers5"] = $teachers;

		$messages= $this->db->query("SELECT * FROM teacher_student_message WHERE student_id = '$student_id'")->result();
		$this->data["messages"] = $messages;
		$comments= $this->db->query("SELECT * FROM comments WHERE student_id = '$student_id'")->result();
		$this->data["comments"] = $comments;
		
		$files = $this->db->query("SELECT * FROM teachers_files WHERE course_id = '$course_id'")->result();
		$i =  0;
		foreach($files as $f){
			$teacher = $this->db->query("SELECT * FROM teachers WHERE id = '$f->teacher_id'")->result();
			$files[$i]->teacher_name = $teacher[0]->full_name;
			
			
			
			$course_id = $f->course_id;
			$course_name= $this->db->query("SELECT * FROM courses WHERE id = '$course_id'")->result();
			$files[$i]->course_name = $course_name[0]->name;
			
			$course_type_name = $this->db->query("SELECT * FROM courses_types WHERE id = (SELECT course_type FROM courses WHERE id = '$course_id')")->result();
			$files[$i]->course_type_name = $course_type_name[0]->ar_name;
			$i++;
		}
	...
	public function join_session($id){
		//echo $id;
		$student_id = $this->session->userdata("student_user_id");
		$session_id = $this->db->query("SELECT * FROM lessons_sessions WHERE time_table_id = '$id' AND end_time = '0000-00-00 00:00:00'")->result();
		$session_id = $session_id[0]->id;
		$this->db->query("INSERT INTO attendes (student_id, date, session_id) VALUES ('$student_id', NOW(), '$session_id')");
		
		header("Location: https://app.bbbserver.eu/de/quick-connect/739831c0-e63a-4eac-b3af-50a72a77506a");
	}
	
	
	public function send_teacer_messaage_page($teacher_id)
	{
		$data['teacher_id']=$teacher_id;
		$this->load->view('Send_message_for_teacher', $this->data);
	}
	public function send_teacer_messaage()
	{
		
		if($this->input->post())
		{
		    $data['student_id']=$this->session->userdata('student_user_id');
			...
			if($response==true)
			{
			    redirect(base_url().'master/teachers/?msg=send_msg_done', 'refresh');
			}
			else{
	...
	public function teachers_comments(){
		
		
		$this->load->view('teachers_comments', $this->data);
	}
	...
	public function personal_info(){
		...
		$this->load->view('personal_info', $this->data);
	}
	...
	public function edit_student_info_done(){
		...
		redirect(base_url().'master/personal_info/?msg=done', 'refresh');
	}
	
	public function add_complaint(){
		$data['student_id']=$this->session->userdata('student_user_id');
			$data['teacher_id']=$this->input->post('teacher_id');
			$data['compliant']=$this->input->post('compliant');
		
			$data['date']=date("Y-m-d");
			$response=$this->db->insert('teachers_compliants',$data);
			if($response==true)
			{
			    redirect(base_url().'master/teachers/?msg=send_compliant_done', 'refresh');
			}
	}
	...
	public function reply_messaage_page($id)
	{
		...
		$teacher_id=$message[0]->teacher_id;
		$this->data["message"] = $message;
		$teacher = $this->db->query("SELECT * FROM teachers WHERE id = '$teacher_id'")->result();
		$this->data["teacher"] = $teacher;
		$this->load->view('reply_messaage_page', $this->data);
	}
	public function reply_messaage($message_id)
	{
		$this->load->database();
		$message = $this->db->query("SELECT * FROM teacher_student_message WHERE id = '$message_id'")->result();	
		if($this->input->post())
		{
			...
			$data['date']=date("Y-m-d");
			$response=$this->db->insert('teacher_student_message',$data);
			//redirect(base_url().'home/reply_messaage_page', 'refresh');
			if($response==true)
			{
	...
	public function new_school_done(){
		...
		foreach($all_settings as $s){
			$this->db->query("INSERT INTO settings (name, school_id, ar_name, type) VALUES ('$s->name', '$last_id', '$s->ar_name', '$s->type')");
		}
	...
	public function new_time_table(){
		...
		$this->db->query("INSERT INTO time_table (course_id, teacher_id, start_time,
			end_time, title, ordering, day) VALUES
			('$course_id', '$teacher_id', '$start_time', '$end_time', '$title', '$ordering', '$day')");
	...
	function fetch_time_table($day, $course_id){
		...
		course_id = '$course_id' AND day = '$day' order BY ordering")->result();
		$i = 0;
	...
	public function certificates(){
	    ...
	    foreach($certificates as $c){
	        ...
	        $student = $this->db->query("SELECT * FROM students WHERE id = '$sid'")->result();
	        $certificates[$i]->student_name = $student[0]->first_name . ' ' . $student[0]->last_name;
	        
	        $course = $this->db->query("SELECT * FROM courses WHERE id = '$cid'")->result();
	        $certificates[$i]->course_name = $course[0]->name;
	...
	public function new_certificate(){
	    extract($_POST);
	    $cert_number = $student_id .'-'.$course_id.'-'.rand(1000, 10000);
	    $this->db->query("INSERT INTO certificates (student_id, course_id, date, cert_number) VALUES ('$student_id', '$course_id', NOW(), '$cert_number' )");
	...
	public function view_exam($id){
		...
		$this->data['exam'] = $this->db->query("SELECT * FROM exams WHERE id = '$id'")->result();
		$this->data['exam_id'] = $id;
		$this->load->view('view_exam', $this->data);
	}
	
	
	public function exams(){
		$student_id = $this->session->userdata("student_user_id");
		$exams = $this->db->query("SELECT * FROM exams WHERE course_id IN (SELECT course_id FROM
			students_courses WHERE student_id = '$student_id') AND active = 1 ORDER BY id DESC")->result();
		$i = 0;
		foreach($exams as $e){
	...
	public function add_new_exam(){
	    ...
	    $this->db->query("INSERT INTO exams (title, details, active, course_id) 
			VALUES ('$title', '$details', 0, '$course_id' )");
	...
	public function questions_bank(){
		$this->data["questions"] = $this->db->query("SELECT * FROM questions_bank")->result();
	...
	public function courses_enrolls_requests(){
	    ...
	    foreach($req as $r){
	        ...
	        if($student){
    	        ...
    	        $req[$i]->email = $student[0]->email;
    	        $req[$i]->phone = $student[0]->mobile;
    	        $req[$i]->stundet_id = $student[0]->id;
	        }
	        else{
	            $req[$i]->student ='';
    	        $req[$i]->email = '';
	...
	public function add_teacher(){
		...
		$this->db->query("INSERT INTO teachers (full_name , details, image, main_subject_id) VALUES ('$full_name', '$details', '$target_path', '$main_subject_id')");
	...
	public function new_book(){
		...
		$main_subject_id = $this->input->post("main_subject_id");
		    ...
		    $this->db->query("INSERT INTO books (name,  course_type_id, link, image, main_subject_id ) 
			VALUES ('$name', '$course_type_id', '$target_path2', '$target_path', '$main_subject_id')");
	...
	public function payments(){
		...
		LEFT JOIN users as user ON user.id = payment.user_id WHERE student.id = '$student_id'";
	...
	public function courses(){
		...
		foreach($courses as $c){
			...
			$cid = $c->course_type;
			$school_name = $this->db->query("SELECT * FROM schools WHERE id = '$sid'")->result();
			$course_type = $this->db->query("SELECT * FROM courses_types WHERE id = '$sid'")->result();
			if($school_name && $course_type){
