...
//print_r($data_search); exit;
		...
		$result = $this->Questionbank_model->getAllData($data_search , 'result', $limit , $start,$id);
		...
		$config['total_rows'] =/*ceil(*/$this->Questionbank_model->getAllData($data_search,'total',$item_per_page,$postion_row,$id);//$item_per_page)-1;
...
//print_r($data); exit;
	...
	public function add() {
		$data['course_types'] = $this->Questionbank_model->get_course_type();
		$data['courses'] = $this->Course->get_courses();
		$data['questions_groups'] = $this->Questionbank_model->get_select('courses',array());
		//	print_r($data); exit;
		$vars['right_menu']= $this->Users_per->fetch_right_menu();
...
//                    TODO fix how the error appeared
	...
	//            add rules for form validation
			...
			if($data['ques_type']=='free'){
				...
				$parent_question['course_id']= $data['listquestion_subject_id'];
...
//                    $tosave['brands_id'] = $comma_separated;
	...
	public function getchild_groups(){

		$data=$this->Questionbank_model->get_select('courses',array('course_type'=>$_GET['class_id']));
		$response = array();
		foreach ($data as $option) {
			$response[] = array(
				'id' => $option['id'],
	...
	public function getchild_subject(){

		$data=$this->Questionbank_model->get_select('main_subjects',array('course_type'=>$_GET['class_id']));
		$response = array();
		foreach ($data as $option) {
			$response[] = array(
				'id' => $option['id'],
