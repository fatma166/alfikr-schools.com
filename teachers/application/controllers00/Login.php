...
class Login extends CI_Controller {
	...
	public function index()
	{
		...
		if($this->session->userdata('teahcer_id')) { 
	...
	public function new_account_done(){
		...
		if(empty($query)){
			...
			if(empty($query)){
				...
				//echo $admin_user_id;
				$this->session->set_userdata('teacher_id', $admin_user_id);
				redirect(base_url().'home', 'refresh');
			}
	...
	public function check(){
		...
		//echo $username;
		$password = md5($this->input->post('password'));
		$query = $this->db->query("SELECT * FROM teachers WHERE email = '$username' AND password = '$password'");
		$query = $query->result();
		if(empty($query)){
			redirect(base_url(), 'refresh');
			//echo base_url();
