...
class Ion_auth
{
	...
	{
        $this->db->select('g.id');
        $this->db->from('users_groups u');
        $this->db->join('groups g', 'u.group_id = g.id', 'left');
        $this->db->where('u.user_id = '.$this->ion_auth->get_user_id());
	...
	public function is_autho($ptype='global_access', $cont='', $return = false)
	{
		if(empty($cont)) $cont = $this->router->class;
		$id = $this->session->userdata('user_id');
			
		if (isset($this->_cache_user_perm[$cont.'_'.$ptype.'_'.$id]))
		{
			$ga = $this->_cache_user_perm[$cont.'_'.$ptype.'_'.$id];
		}
		else
		{

			$this->db->select('p.permission_type as action');
			$this->db->from('users_groups u');
			$this->db->join('crud_permissions p', 'u.group_id = p.group_id', 'left');
			$this->db->where('u.user_id = '.$id);
			$this->db->where('p.com_id',$cont);
			$ga = $this->db->get()->result_array();
			$this->_cache_user_perm[$cont.'_'.$ptype.'_'.$id] = $ga;
		}
		...
		if(!empty($ga)){
			...
			if(!in_array('global_access', $gac) || !in_array($ptype, $gac) ){
				if($return){return false;}else{show_error('Permission error : you can\'t '.$ptype.' On  '.$cont);}
				}else{
					return true;
				}
		}else{
	...
	public function nextGroup($gid){
	
		
		$this->db->select('b.id');
		$this->db->from('groups a');
		$this->db->join('groups b', 'b.group_p = a.group_p + 1', 'left');
		$this->db->where('a.id = '.$gid);
		
		return $this->db->get()->row()->id;
	}
	
	public function prevGroup($gid){
	...
	public function in_group($check_group, $id=false, $check_all = false)
	{
		...
		else
		{
			...
			{
				$groups_array[$group->id] = $group->group_manage_flag;
			}
		...
		foreach ($check_group as $key => $value)
		{
			 ...
			 * if all, !in_array
			 */
			if (in_array(1, $groups) xor $check_all)
			{
				/**
				 * if !all (default), true
				 * if all, false
